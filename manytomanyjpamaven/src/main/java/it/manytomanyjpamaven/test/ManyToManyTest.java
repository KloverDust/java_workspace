...
public class ManyToManyTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//testRimuoviRuoloConUtentiAssociati(ruoloServiceInstance, utenteServiceInstance);
			testRimuoviRuoloSenzaUtentiAssociati(ruoloServiceInstance);
	...
	private static void testRimuoviRuoloDaUtente(RuoloService ruoloServiceInstance, UtenteService utenteServiceInstance)
		...
		Ruolo ruoloEsistenteSuDb = ruoloServiceInstance.cercaPerDescrizioneECodice("Visitor", "ROLE_VISITOR");
		if (ruoloEsistenteSuDb == null)
	...
	private static void testRimuoviRuoloSenzaUtentiAssociati(RuoloService ruoloService) throws Exception {
		System.out.println(".......testDeleteRuoloSenzaUtentiAssociati inizio.............");

		Ruolo ruolo = new Ruolo("Ruolo Temp Libero", "ROLE_FREE");
		ruoloService.inserisciNuovo(ruolo);
		Long idRuolo = ruolo.getId();
		if (idRuolo == null)
			throw new RuntimeException("Ruolo non inserito correttamente");

		ruoloService.rimuovi(idRuolo);
		System.out.println(".......testDeleteRuoloSenzaUtentiAssociati fine: PASSED.............");
	}

	private static void testRimuoviRuoloConUtentiAssociati(RuoloService ruoloService, UtenteService utenteService) throws Exception {
		System.out.println(".......testDeleteRuoloConUtentiAssociati inizio.............");

		Ruolo ruolo = new Ruolo("Ruolo Occupato", "ROLE_USED");
		ruoloService.inserisciNuovo(ruolo);

		Utente utente = new Utente("user.temp", "pass", "temp", "user", LocalDate.now());
		utenteService.inserisciNuovo(utente);
		utenteService.aggiungiRuolo(utente, ruolo);

		try {
			ruoloService.rimuovi(ruolo.getId());
			throw new RuntimeException("ERRORE: doveva essere sollevata un'eccezione UtenteConRuoliAssociatiException");
		} catch (UtenteConRuoliAssociatiException e) {
			// eccezione attesa, test passato
		}

		System.out.println(".......testDeleteRuoloConUtentiAssociati fine: PASSED.............");
	}
